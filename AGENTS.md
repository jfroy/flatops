...
- topf does **not** set the hostname from `host`; `all/05-hostname.yaml.tpl` does that. Without it Talos renames nodes to `talos-xxx-xxx`.
...
- Config format: the fields Talos 1.13.7 deprecates **and** provides a document kind for are already multi-doc (`HostnameConfig`, `ResolverConfig`, `DHCPv4Config`/`LinkConfig`, `VolumeConfig`, `RegistryMirrorConfig`). Everything else is still v1alpha1 because the replacement kinds (`UnattendedInstallConfig`, `KubeletConfig`, `KubeNodeConfig`, `SysctlConfig`, `KernelModuleConfig`, the `Kube*Config` family) only exist from 1.14 — a 1.13.7 node rejects them as not registered. A v1alpha1 field and its document are mutually exclusive, so converting one is a move, not an addition. See [docs/talos-1.14-migration.md](docs/talos-1.14-migration.md).
